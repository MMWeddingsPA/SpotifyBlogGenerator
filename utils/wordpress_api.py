...
class WordPressAPI:
    def __init__(self, api_url, username, password):
        ...
        # Set up authentication header only (simplifying the authentication approach)
    ...
    def test_connection(self):
        ...
        try:
            ...
            auth_token = headers.get('Authorization', '').replace('Basic ', '')
            curl_cmd = f'curl -v -X GET "{endpoint}" -H "Accept: application/json" -H "User-Agent: WordPress API Client/1.0" -H "Authorization: Basic {auth_token}"'
            logger.info(f"Equivalent curl command: {curl_cmd.replace(auth_token, '[REDACTED]')}")
    ...
    def create_post(self, title, content, status='draft', featured_media=None, categories=None, tags=None):
        ...
        try:
            ...
            auth_token = headers.get('Authorization', '').replace('Basic ', '')
            curl_cmd = (
                f'curl -v -X POST "{endpoint}" '
                f'-H "Accept: application/json" '
                f'-H "Content-Type: application/json" '
                f'-H "User-Agent: WordPress API Client/1.0" '
                f'-H "Authorization: Basic {auth_token}" '
                f'-d \'{json.dumps(post_data)}\''
            )
            logger.info(f"Equivalent curl command: {curl_cmd.replace(auth_token, '[REDACTED]')}")
